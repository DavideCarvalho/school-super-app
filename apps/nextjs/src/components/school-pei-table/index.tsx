...
import { EditPeiModal } from "../edit-pei-modal";
import { NewPeiModal } from "../new-pei-modal";
...
export function PeiTable({ schoolId }: PeiTableProps) {
  const router = useRouter();
  const { user } = useUser();

  const [selectedPei, setSelectedPei] = useState<Class>();

  const [open, setOpen] = useState(false);
  const [openEditModal, setOpenEditModal] = useState(false);

  const epiQuery = api.pei.allBySchoolId.useQuery(
    {
      schoolId,
  ...
  const epiCountQuery = api.pei.countAllBySchoolId.useQuery(
    { schoolId },
    { refetchOnMount: false },
  );
  ...
  function onSelectPeiToEdit(schoolClass: Class) {
    setOpenEditModal(true);
    setSelectedPei(schoolClass);
  }

  async function onEdited() {
    setOpenEditModal(false);
    setSelectedPei(undefined);
    await epiQuery.refetch();
    await epiCountQuery.refetch();
  }
    ...
    <div className="bg-white py-12 sm:py-16 lg:py-20">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        ...
        <div className="divide-y divide-gray-200">
          ...
          {!epiQuery.isFetching &&
            epiQuery.data?.map((pei) => {
              return (
                <TableRow
                  key={pei.id}
                  pei={pei}
                  onDelete={deleteClass}
                  onEdit={(pei) => onSelectPeiToEdit(pei)}
                />
              );
            })}
        </div>
        ...
        <div>
          <Pagination
            totalCount={epiCountQuery?.data ?? 0}
...
function TableRow({ pei, onDelete, onEdit }: TableRowProps) {
