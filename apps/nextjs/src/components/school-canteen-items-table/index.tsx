...
import type { Canteen, CanteenItem, User } from "@acme/db";
...
export function SchoolCanteenItemsTable({
  ...
  const canteenId = user?.publicMetadata?.canteen?.id;
  ...
  const allBySchoolIdQuery = api.canteen.allItemsBySchoolId.useQuery(
    {
      canteenId,
  ...
  const countAllBySchoolIdQuery = api.canteen.countAllItemsBySchoolId.useQuery(
    { canteenId },
    { refetchOnMount: false },
  );
