...
export default function CanteensPage({
  ...
  return (
    <SchoolLayout>
      <SchoolCanteenItemsTable schoolId={school.id} />
    </SchoolLayout>
  );
}
...
export const getServerSideProps = wrapGetServerSidePropsWithSentry(
  async ({ req, params, query }: GetServerSidePropsContext) => {
    ...
    await Promise.all([
      serverSideHelpers.canteen.allItemsBySchoolId.prefetch({
        schoolId: school.id,
        page,
        limit,
      }),
      serverSideHelpers.canteen.countAllItemsBySchoolId.prefetch({
        schoolId: school.id,
      }),
    ]);
