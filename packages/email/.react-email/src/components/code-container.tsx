...
export const CodeContainer: React.FC<Readonly<CodeContainerProps>> = ({
  ...
  const renderDownloadIcon = () => {
    ...
    return (
      <a
        href={url}
        download={file.name}
        className="text-slate-11 hover:text-slate-12 transition duration-200 ease-in-out"
      >
        <IconDownload />
      </a>
    );
  ...
  return (
    ...
    >
      <div className="border-slate-6 h-9 border-b">
        <div className="flex">
          <LayoutGroup id="code">
            {markups.map(({ language }) => {
              ...
              return (
                <motion.button
                  className={`hover:text-slate-12 relative py-[8px] px-4 font-sans text-sm font-medium transition duration-200 ease-in-out ${
